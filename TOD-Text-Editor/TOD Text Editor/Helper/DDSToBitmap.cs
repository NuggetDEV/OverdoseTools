...
namespace Helper
{
    public class DDSToBitmap
    {
        ...
        #region DXT1
        static private Bitmap UncompressDXT1(byte[] data, int w, int h)
        {
        ...
        //from http://code.google.com/p/kprojects
        #region DXT5
        static private Bitmap UncompressDXT5(byte[] data, int w, int h)
        {
            Bitmap res = new Bitmap((w < 4) ? 4 : w, (h < 4) ? 4 : h);
            using (MemoryStream ms = new MemoryStream(data))
            {
        ...
        #region DXT3
        private static Bitmap UncompressDXT3(byte[] imgData, int w, int h)
        {
        ...
        static private Bitmap Uncompress8_8_8_8(byte[] data, int w, int h)
        {
            ...
            for (int y = 0; y < bitmap.Height; y++)
            {
                ...
                {
                    uint RGBABits = BitConverter.ToUInt32(data, Step);
                    Step += sizeof(uint);
        ...
        #region   A8L8
        static private Bitmap UncompressA8L8(byte[] data, int w, int h)
        {
            Bitmap bitmap = new Bitmap(w, h);
            int Step = 0;
            for (int y = 0; y < bitmap.Height; y++)
            {
                for (int x = 0; x < bitmap.Width; x++)
                {
                    uint RGBABits = BitConverter.ToUInt16(data, Step);
                    Step += sizeof(ushort);
        ...
        #region   V8U8
        static private Bitmap UncompressV8U8(byte[] data, int w, int h)
        {
            Bitmap bitmap = new Bitmap(w, h);
            int Step = 0;
            for (int y = 0; y < bitmap.Height; y++)
        ...
        #region   B4G4R4A4
        static private Bitmap UncompressB4G4R4A4(byte[] data, int w, int h)
        {
            Bitmap bitmap = new Bitmap(w, h);
            int Step = 0;
            for (int y = 0; y < bitmap.Height; y++)
        ...
        #region   B5G5R5A1
        static private Bitmap UncompressB5G5R5A1(byte[] data, int w, int h)
        {
            Bitmap bitmap = new Bitmap(w, h);
            int Step = 0;
            for (int y = 0; y < bitmap.Height; y++)
        ...
        #region L8 or A8
        static private Bitmap Uncompress8bits(byte[] data, int w, int h)
        {
            Bitmap bitmap = new Bitmap(w, h);
            int Step = 0;
            for (int y = 0; y < bitmap.Height; y++)
        ...
        public static Bitmap Convert(byte[] DDSBytes)
        {
            ...
            if (info.Format == DDSFormat.bc1_unorm || info.Format == DDSFormat.bc1_unorm_srgb)
            {
                return UncompressDXT1(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            if (info.Format == DDSFormat.bc2_unorm || info.Format == DDSFormat.bc2_unorm_srgb)
            {
                return UncompressDXT3(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            if (info.Format == DDSFormat.bc3_unorm || info.Format == DDSFormat.bc3_unorm_srgb)
            {
                return UncompressDXT5(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            if (info.Format == DDSFormat.b8g8r8a8_unorm
             || info.Format == DDSFormat.b8g8r8x8_unorm
             || info.Format == DDSFormat.r8g8b8a8_unorm
             || info.Format == DDSFormat.r8g8b8a8_unorm_srgb
             || info.Format == DDSFormat.b8g8r8a8_unorm_srgb
             || info.Format == DDSFormat.b8g8r8x8_unorm_srgb)
            {
                return Uncompress8_8_8_8(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            if (info.Format == DDSFormat.r8g8_unorm)
            {
                return UncompressA8L8(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            if (info.Format == DDSFormat.r8g8_snorm)
            {
                return UncompressV8U8(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            if (info.Format == DDSFormat.r8_unorm || info.Format == DDSFormat.a8_unorm)
            {
                return Uncompress8bits(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            if (info.Format == DDSFormat.b4g4r4a4_unorm || info.DDS_HEADER.ddspf.RGBBitCount == 16)
            {
                return UncompressB4G4R4A4(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            if (info.Format == DDSFormat.b5g5r5a1_unorm)
            {
                return UncompressB5G5R5A1(info.Data, info.DDS_HEADER.width, info.DDS_HEADER.height);
            }

            throw new Exception("Unsupported DDS Format: " + info.Format.ToString());
        }
        ...
        public static Bitmap Convert(byte[] DDSBytes, DDSFormat format, int w, int h)
        {
            ...
            (uint RBitMask, uint GBitMask, uint BBitMask, uint ABitMask) = DDSCooker.GetMask(info.Format);
            info = new DDS_Info()
            {
